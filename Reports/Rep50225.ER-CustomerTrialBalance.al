report 50225 "ER - Customer Trial Balance"
{
    ApplicationArea = All;
    Caption = 'ER - Customer Trial Balance';
    DefaultLayout = RDLC;
    RDLCLayout = 'Reports Layouts/ER-CustomerTrialBalance.rdl';
        ...
        dataitem(Customer; Customer)
        {
            ...
            dataitem(TempCurrencyTotal; "AC Currency Total")
            {
                ...
                begin
                    ...
                    Cust.Reset();
                    Cust.SetFilter("No.", Customer."No.");
                    Cust.SetFilter("Currency Filter", TempCurrencyTotal.CurrencyCode);
                    Cust.SetFilter("Date Filter", '<' + format(FromDate));

                    if Cust.FindFirst() then begin
                        Cust.CalcFields("Net Change", "Net Change (LCY)");
                        BeginBalOrig := Cust."Net Change";
                        BeginBalLCY := Cust."Net Change (LCY)";
                    end;
                    ...
                    Cust.SetFilter("No.", Customer."No.");
                    Cust.SetFilter("Currency Filter", TempCurrencyTotal.CurrencyCode);
                    Cust.SetFilter("Date Filter", format(FromDate) + '..' + format(ToDate));

                    if Cust.FindFirst() then begin
                        Cust.CalcFields("Net Change", "Net Change (LCY)", "Credit Amount", "Credit Amount (LCY)", "Debit Amount", "Debit Amount (LCY)");

                        DebiOrig := Cust."Debit Amount";
                        DebitLCY := Cust."Debit Amount (LCY)";
                        CreditOrig := Cust."Credit Amount";
                        CreditLCY := Cust."Credit Amount (LCY)";
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    {
                        ApplicationArea = all;
                        Caption = 'Hide Customers With no Balance';
                    }
