page 50234 "RM per Sales Quote SD"
{
    ApplicationArea = All;
    Caption = 'RM per Sales Quote SD';
    PageType = StandardDialog;
    SourceTableView = sorting("Req Qty is Missing") order(descending);
    ...
    layout
    {
        area(Content)
        {
            ...
            grid(Footer)
            {
                ...
                GridLayout = Columns;
                label(" ") { Visible = false; }
                label("Do you want to Make Order with Missing Quantity ?")
                {
                    Style = Unfavorable;
                    Caption = 'Do you want to Make Order with Missing Quantity ?';
                }
                label("  ") { Visible = false; }
            }
