page 50256 "Plotting File FactBox"
{
    ...
    layout
    {
        area(content)
        {
            field(Picture; Rec.Picture)
            {
                ...
                ShowCaption = false;
                ToolTip = 'Specifies the picture that has been inserted for the Plotting.';
            }
        }
    }
    ...
    actions
    {
        area(processing)
        {
            ...
            action("Import Picture")
            {
                ...
                begin
                    Rec.TestField("Design Plotting ID");
                    //Rec.TestField(Width);
    ...
    begin
        ...
        OnAfterDeletePlottingPicture(Rec);
    end;
            ...
            for index := 1 to Rec.Picture.COUNT do begin
                if Media.Get(Rec.Picture.Item(index)) then begin
                    ...
                    if Media.Content.HasValue() then begin
                        ...
                        FileName := Format(Rec."Design Plotting ID") + GetImgFileExt(Mime);
                    end;
                end;
            end;
    end;
    ...
    local procedure OnAfterDeletePlottingPicture(var PlottingImage: Record "Plotting File")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterTakeNewPicture(var PlottingImage: Record "Plotting File"; IsPictureAdded: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnImportFromDeviceOnAfterModify(var PlottingImage: Record "Plotting File")
    begin
    end;
