report 50250 "Revalue Items 2023"
{
    ...
    begin
        // Skip entries with zero quantity or already revalued
        if (ItemLedgerEntry.Quantity = 0) or (ItemLedgerEntry."Completely Invoiced" = false) then
            exit;

        // Get cost amounts from value entries
        ...
        // Check if we need to revalue based on LCY to ACY ratio
        if (CostAmountACY <> 0) and ((CostAmountActual / CostAmountACY) < 42000) then begin
            // Calculate the desired ratio based on conditions
            if (ItemLedgerEntry."Remaining Quantity" > 0) then
                // Special rate for remaining quantities
                NewUnitCost := 89500 / ItemLedgerEntry.Quantity
            else if (CostAmountACY >= 42000) then
                NewUnitCost := CostAmountACY / ItemLedgerEntry.Quantity
            else
                NewUnitCost := 42000 / ItemLedgerEntry.Quantity;

            // Calculate exchange rate for reference
            Rate := CostAmountActual / CostAmountACY;

            // Create the revaluation entry
    ...
    begin
        ...
        // Calculate current unit cost from value entries
        ValueEntry.SetRange("Item Ledger Entry No.", ItemLedgerEntry."Entry No.");
        if ValueEntry.FindSet() then
            repeat
                CostAmountActual += ValueEntry."Cost Amount (Actual)";
        ...
        // Check if cost amount ACY divided by quantity is less than 42,000
        CostPerUnit := CostAmountACY / ItemLedgerEntry.Quantity;

        // Check if cost amount LCY divided by cost amount ACY is greater than or equal to 42000
        // if (CostAmountACY <> 0) and ((CostAmountActual / CostAmountACY) >= 42000) then imppppp
        //     exit; // Skip if cost amount LCY / cost amount ACY >= 42000

        // We want (CostAmountActual + AdjustmentValue) / CostAmountACY = 42000
        // So: AdjustmentValue = (42000 * CostAmountACY) - CostAmountActual
        if (CostPerUnit < 42000) then begin
            // Calculate the value needed to make the ratio equal to 42000
            InventoryValueToSet := (42000 * CostAmountACY) - CostAmountActual;

        end else begin
            exit; // No journal line needed if cost per unit is >= 42,000
        end;
