page 50271 "Possible Value Picture"
{
    Caption = 'Possible Value Picture';
    DeleteAllowed = false;
    InsertAllowed = false;
    LinksAllowed = false;
    PageType = CardPart;
    ...
    layout
    {
        area(content)
        {
            field(Picture; Rec.Picture)
            {
                ...
                ShowCaption = false;
                ToolTip = 'Specifies the picture that has been inserted for the Feature.';
            }
        }
    }
    ...
    begin
        ...
        OnAfterDeleteFeaturePicture(Rec);
    end;
            ...
            for index := 1 to Rec.Picture.COUNT do begin
                if Media.Get(Rec.Picture.Item(index)) then begin
                    ...
                    if Media.Content.HasValue() then begin
                        ...
                        FileName := Rec."Feature Name" + '' + GetImgFileExt(Mime);
                    end;
                end;
            end;
    end;
    ...
    local procedure OnAfterDeleteFeaturePicture(var ItemFeaturePossibleValue: Record "Item Feature Possible Values")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterTakeNewPicture(var ItemFeaturePossibleValue: Record "Item Feature Possible Values"; IsPictureAdded: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnImportFromDeviceOnAfterModify(var ItemFeaturePossibleValue: Record "Item Feature Possible Values")
    begin
    end;
